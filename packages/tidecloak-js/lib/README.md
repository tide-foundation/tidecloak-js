...
The hashes pin the page's inline script and style, so they change when the page does.

If the page is missing, login still works but stalls for about four seconds and falls back to a popup. The SDK logs the URL it expected at startup.

#### 2. Set up correct authenticator flow on Tidecloak

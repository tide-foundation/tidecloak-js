...
export default class TideCloak {
  ...
  async #processCallback (oauth) {
    ...
    const authSuccess = (accessToken, refreshToken, idToken, dokenValue) => {
      ...
      this.#setToken(accessToken, refreshToken, idToken, timeLocal, dokenValue)

      if (this.#useNonce && (this.idTokenParsed && this.idTokenParsed.nonce !== oauth.storedNonce)) {
        this.#logInfo('[TIDECLOAK] Invalid nonce, clearing token')
        this.clearToken()
  ...
  isTokenExpired = (minValidity) => {
    ...
    if (this.timeSkew == null) {
      this.#logInfo('[TIDECLOAK] Unable to determine if token is expired as timeskew is not set')
      return true
    }
  ...
  updateToken = async (minValidity) => {
    ...
    if (minValidity === -1) {
      refreshToken = true
      this.#logInfo('[TIDECLOAK] Refreshing token: forced refresh')
    } else if (!this.tokenParsed || this.isTokenExpired(minValidity)) {
      refreshToken = true
      this.#logInfo('[TIDECLOAK] Refreshing token: token expired')
    }
    ...
    if (this.#refreshQueue.length === 1) {
      ...
      try {
        ...
        this.#setToken(response.access_token, response.refresh_token, response.id_token, timeLocal, response.doken)
        ...
        this.#logWarn('[TIDECLOAK] Failed to refresh token')
  ...
  initRequestEnclave () {
    ...
    if (!this.requestEnclave) {
      ...
      console.log("[TIDECLOAK] Successfully started RequestEnclave")
    }
  }
  ...
  #getVoucherUrl () {
    ...
    const realmUrl = this.#getRealmUrl()
    if (!realmUrl) throw new Error('Unable to build voucher URL, realm URL not configured')
    ...
    if (token) {
      ...
      if (typeof this.timeSkew === 'number') {
        ...
        if (this.onTokenExpired) {
          ...
          this.#logInfo('[TIDECLOAK] Token expires in ' + Math.round(expiresIn / 1000) + ' s')
          if (expiresIn <= 0) {
            this.onTokenExpired()
          } else {

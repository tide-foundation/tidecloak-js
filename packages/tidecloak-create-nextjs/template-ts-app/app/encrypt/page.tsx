...
export default function EncryptPage() {
  ...
  const [committedRoles, setCommittedRoles] = useState<{ encryptRole: string | null; decryptRole: string | null } | null>(null)
  ...
  const note = (text: string, type: 'info' | 'success' | 'error' = 'info') => setMsg({ text, type })
  ...
  async function refresh() {
    try {
      ...
      const mine = (committedRes as CommittedPolicy[]).find((p) => p.contractId === contractId)
      if (mine) {
  ...
  async function handleApprove(p: PendingPolicy) {
    setBusy('approve')
    try {
  ...
  async function handleCommit(p: PendingPolicy) {
    setBusy('commit')
    try {
      ...
      const signatures: Uint8Array[] = await (IAMService as any)._tc.executeSignRequest(req.encode(), true)
  ...
  async function handleEncrypt() {
    ...
    try {
      ...
      const results: string[] = await (IAMService as any).doEncrypt(
  ...
  async function handleDecrypt() {
    ...
    setBusy('decrypt')
    try {
      const results: (string | Uint8Array)[] = await (IAMService as any).doDecrypt(
...
const page: CSSProperties = { minHeight: '100vh', display: 'flex', justifyContent: 'center', background: '#f5f5f5', padding: '2rem' }
const card: CSSProperties = { background: '#fff', padding: '2rem', borderRadius: 8, boxShadow: '0 4px 12px rgba(0,0,0,0.1)', maxWidth: 640, width: '100%' }
const section: CSSProperties = { borderTop: '1px solid #eee', marginTop: '1.25rem', paddingTop: '1.25rem' }
const h2: CSSProperties = { fontSize: '1.05rem', margin: '0 0 0.5rem' }
const subtle: CSSProperties = { color: '#777', fontSize: '0.85rem', fontWeight: 400 }
const okText: CSSProperties = { color: '#176c3a', fontSize: '0.95rem' }
const label: CSSProperties = { display: 'block', fontSize: '0.85rem', color: '#444', marginTop: '0.6rem' }
const input: CSSProperties = { display: 'block', width: '100%', padding: '0.5rem', marginTop: '0.25rem', borderRadius: 4, border: '1px solid #ccc', boxSizing: 'border-box' }
const textarea: CSSProperties = { ...input, minHeight: 64, fontFamily: 'inherit' }
const primaryBtn: CSSProperties = { marginTop: '0.75rem', padding: '0.6rem 1.1rem', fontSize: '0.95rem', borderRadius: 4, border: 'none', background: '#0070f3', color: '#fff', cursor: 'pointer' }
const disabledBtn: CSSProperties = { ...primaryBtn, background: '#cfcfcf', color: '#fff', cursor: 'not-allowed' }
const ghostBtn: CSSProperties = { padding: '0.4rem 0.8rem', fontSize: '0.85rem', borderRadius: 4, border: '1px solid #ccc', background: '#fff', color: '#333', cursor: 'pointer' }
const stepRow: CSSProperties = { display: 'flex', gap: '0.7rem', alignItems: 'flex-start', padding: '0.6rem 0', borderTop: '1px solid #f0f0f0' }
const badgeBase: CSSProperties = { flex: '0 0 auto', width: 24, height: 24, borderRadius: 12, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.8rem', fontWeight: 700, color: '#fff', marginTop: 2 }
const badgeDone: CSSProperties = { ...badgeBase, background: '#1a9c4e' }
const badgeActive: CSSProperties = { ...badgeBase, background: '#0070f3' }
const badgeLocked: CSSProperties = { ...badgeBase, background: '#bbb' }
const banner: CSSProperties = { marginTop: '1rem', padding: '0.6rem 0.8rem', borderRadius: 4, background: '#eef3ff', fontSize: '0.9rem' }
const bannerErr: CSSProperties = { background: '#fdecea', color: '#a32219' }
const bannerOk: CSSProperties = { background: '#e9f7ef', color: '#176c3a' }
const pre: CSSProperties = { marginTop: '0.5rem', padding: '0.6rem', background: '#f4f4f4', borderRadius: 4, fontSize: '0.8rem', whiteSpace: 'pre-wrap', wordBreak: 'break-all' }

...
export default class IAMService {
   ...
   *
   * @param {Function} onReady - Callback invoked when the SSO check completes.                           Receives `true` if authenticated, otherwise `false`.
   * @param {string} pathToConfig - path from root to tidecloak client adapter configuration.
   */
  initIAM(pathToConfig, onReady) {
    ...
    if (kc === null) {
      this.loadConfig(pathToConfig).then(() => {
  ...
  async loadConfig(pathToConfig) {
    ...
    try {
      const module = await import(`${pathToConfig}`);
      kcData = module.default;
    } catch (err) {
...
/**
 * Return the already-loaded config, or throw if none yet.
 * @returns {object}
 */
  static getConfig() {
    if (!this._config) {
      throw new Error(
        "TideCloak config not loaded yet — call IAMService.loadConfig(path) first"
      );
    }
    return this._config;
  }

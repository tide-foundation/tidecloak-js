...
//
import { Serialization } from "../modules/tide-js/Cryptide/index.js";
import { base64ToBytes, CreateTideMemory, StringFromUint8Array, StringToUint8Array } from "../modules/tide-js/Cryptide/Serialization.js";
import { AuthorizedEncryptionFlow } from "../modules/tide-js/Flow/EncryptionFlows/AuthorizedEncryptionFlow.js";
import { AuthorizedSigningFlow } from "../modules/tide-js/Flow/SigningFlows/AuthorizedSigningFlow.js"
...
import { ModelRegistry } from "./heimdall.js";



function Heimdall(config) {
    if (!(this instanceof Heimdall)) {
        throw new Error("The 'heimdall' constructor must be invoked with 'new'.")
    }

    if (typeof config !== 'string' && !isObject(config)) {
        throw new Error("The 'heimdall' constructor must be provided with a configuration object, or a URL to a JSON configuration file.");
    }
    ...
    if (!globalThis.isSecureContext) {
        logWarn(
            "[Heimdall] Heimdall JS must be used in a 'secure context' to function properly as it relies on browser APIs that are otherwise not available.\n" +
    ...
    kc.init = function (initOptions = {}) {
        if (kc.didInitialize) {
            throw new Error("A 'Heimdall' instance can only be initialized once.");
        }
    ...
    kc.encrypt = async function (toEncrypt) {
        ...
        if (!kc.encryptionFlow) kc.encryptionFlow = new AuthorizedEncryptionFlow({
        ...
        return (await kc.encryptionFlow.encrypt(dataToSend)).map(e => bytesToBase64(e));
    }

    function getVoucherUrl() {
        if (!kc.tokenParsed) throw 'User authentication required to access voucher service';
    ...
    kc.decrypt = async function (toDecrypt) {
        ...
        const dataToSend = toDecrypt.map(e => {
            if (!isObject(e)) throw 'All entries must be an object to decrypt';
        ...
        // Construct Tide serialized data payloads
        return (await kc.encryptionFlow.decrypt(dataToSend)).map(d => StringFromUint8Array(d));
    }
    ...
    kc.isTokenExpired = function (minValidity) {
        ...
        if (kc.timeSkew == null) {
            logInfo('[Heimdall] Unable to determine if token is expired as timeskew is not set');
            return true;
        }
    ...
    kc.updateToken = function (minValidity) {
        ...
        var exec = function () {
            ...
            if (minValidity == -1) {
                refreshToken = true;
                logInfo('[Heimdall] Refreshing token: forced refresh');
            } else if (!kc.tokenParsed || kc.isTokenExpired(minValidity)) {
                refreshToken = true;
                logInfo('[Heimdall] Refreshing token: token expired');
            }
            ...
            if (!refreshToken) {
                ...
                if (refreshQueue.length == 1) {
                    ...
                    req.onreadystatechange = function () {
                        if (req.readyState == 4) {
                            if (req.status == 200) {
                                logInfo('[Heimdall] Token refreshed');
                                ...
                                logWarn('[Heimdall] Failed to refresh token');
    ...
    kc.createCardanoTxDraft = function (txBody) {
        const txBodyBytes = Serialization.base64ToBytes(txBody);
        return Serialization.bytesToBase64(CreateTideMemory(txBodyBytes, txBodyBytes.length + 4));
    }
    ...
    kc.signCardanoTx = async function (txBody, authorizers, ruleSettings, expiry) {
        ...
        // Now lets actually encrypt
        const txSigningFlow = new AuthorizedSigningFlow({
            vendorId: config.vendorId,
            token: kc.token,
            voucherURL: getVoucherUrl(),
            homeOrkUrl: config.homeOrkUrl
        ...
        const result = (await txSigningFlow.sign(txBody, authorizers, expiry, ruleSettings))
    ...
    kc.createRuleSettingsDraft = function (ruleSettings, previousRuleSetting, previousRuleSettingCert) {
        ...
        console.log("HERSERWERWE")
        console.log(previousRuleSettingCert)
        ...
        return  Serialization.bytesToBase64(ruleReqDraft.getDraft());
    }
    
    kc.signRuleSettings = async function (ruleReqDraft, authorizers, expiry, initCert) {
        const sessKey = GenSessKey();
        const gSessKey = GetPublic(sessKey);
        ...
        const ruleSignRequest = new RuleSettingsSignRequest("Admin:1");
        ruleSignRequest.setDraft(ruleReqDraft);
        new AuthorizationBuilder(ruleSignRequest, authorizers).addAuthorization();
        ruleSignRequest.setCustomExpiry(expiry);

        // Now lets actually encrypt
        const txSigningFlow = new dVVKSigningFlow(
            config.vendorId,
            vvkInfo.UserPublic,
            vvkInfo.OrkInfo,
            sessKey,
        ...
        // Construct Tide serialized data payloads
        const result = (await txSigningFlow.start(ruleSignRequest));
    ...
    function processCallback(oauth, promise) {
        ...
        function authSuccess(accessToken, refreshToken, idToken, fulfillPromise) {
            ...
            if (useNonce && (kc.idTokenParsed && kc.idTokenParsed.nonce != oauth.storedNonce)) {
                logInfo('[Heimdall] Invalid nonce, clearing token');
                kc.clearToken();
                promise && promise.setError();
            } else {
                if (fulfillPromise) {
    ...
    function setToken(token, refreshToken, idToken, timeLocal) {
        ...
        if (token) {
            ...
            if (kc.timeSkew != null) {
                logInfo('[Heimdall] Estimated time difference between browser and server is ' + kc.timeSkew + ' seconds');

                if (kc.onTokenExpired) {
                    var expiresIn = (kc.tokenParsed['exp'] - (new Date().getTime() / 1000) + kc.timeSkew) * 1000;
                    logInfo('[Heimdall] Token expires in ' + Math.round(expiresIn / 1000) + ' s');
                    if (expiresIn <= 0) {
                        kc.onTokenExpired();
                    } else {
        ...
        if ((loginIframe.enable || kc.silentCheckSsoRedirectUri) && typeof kc.endpoints.thirdPartyCookiesIframe === 'function') {
            ...
            var messageCallback = function (event) {
                ...
                if (event.data !== "supported" && event.data !== "unsupported") {
                    ...
                    logWarn(
                        "[Heimdall] Your browser is blocking access to 3rd-party cookies, this means:\n\n" +
...
export { ModelRegistry } from "../modules/tide-js/Models/ModelRegistry.js"

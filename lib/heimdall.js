...
import { RequestEnclave } from "../node_modules/heimdall-tide/dist/heimdall.js"
import { base64ToBytes, StringFromUint8Array, StringToUint8Array, CreateTideMemory } from "../modules/tide-js/Cryptide/Serialization.js";
...
function TideCloak (config) {    
    ...
    if (isObject(config)) {
        const requiredProperties = 'oidcProvider' in config
            ? ['clientId']
            : ['url', 'realm', 'clientId', 'homeOrkUrl', 'vendorId', 'client-origin-auth-' + window.location.origin];
    ...
    kc.ensureTokenReady = async function() {
        if(kc.isTokenExpired()){
            await kc.updateToken(-1); // ensure token will only be refreshed if it within 20s of expiring
        }
    }
    ...
    kc.encrypt = async function(toEncrypt){
        ...
        return (await this.requestEnclave.encrypt(dataToSend)).map(e => bytesToBase64(e));
    }
    ...
    kc.initEnclave = function(){
        ...
        if(!this.requestEnclave){
            this.requestEnclave = new RequestEnclave({
                ...
                signed_client_origin: config['client-origin-auth-' + window.location.origin],
            ...
            req.onreadystatechange = function () {
                if (req.readyState == 4) {
                    if (req.status == 200) {
                        ...
                        var tokenResponse = JSON.parse(req.responseText);
                        authSuccess(tokenResponse['access_token'], tokenResponse['refresh_token'], tokenResponse['id_token'], kc.flow === 'standard', tokenResponse['doken']);
                        scheduleCheckIframe();
                    } else {
